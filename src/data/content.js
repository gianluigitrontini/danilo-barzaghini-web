export const content = {
  homepage: {
    it: {
      hero: {
        main: `Grazie ad un approccio multidimensionale, tecniche linguistiche e subliminali, ti accompagno verso gli obiettivi desiderati, che si sviluppano dal profondo, si realizzano sulla superficie e si manifestano in nuovi stati percepiti dell'essere.`,
        secondary: `Il rispetto della persona, la qualità dell'ascolto e la capacità di intervenire in modo corretto, contraddistinguono l'efficienza e la qualità del lavoro che svolgo.`,
      },
      reviews: {
        column1: [
          {
            text: 'La collaborazione con Danilo è divertente, chiara e precisa. Mi ha fatto domande chiare e dirette, per focalizzare il lavoro sui temi che voglio cambiare. In poche sedute ho potuto sperimentare profondi cambiamenti a livello fisico ed emotivo. Grazie mille per il suo lavoro. È bello che ci sei.',
            author: 'S.B. - Svizzera',
          },
          {
            text: 'Danilo, ci conosciamo e siamo amici da una vita, ammiro e affascino il tuo percorso, voglio solo dirti un grande GRAZIE per quello che hai fatto al mio subconscio. Le mie abitudini con i dolci sono cambiate, mi sento libera, più energica e in controllo di me stessa, sono io che decido quando e quanto. Non so ancora come hai fatto, ma non mi interessa. Continua ad aiutare chi te lo chiede.',
            author: 'B.B. - Svizzera',
          },
          {
            text: 'Danilo mi ha aiutato con esercizi convincenti a riconoscere e risolvere i modelli nel mio comportamento. Un passo importante per la mia autovalutazione! Molte grazie per il tuo meraviglioso lavoro.',
            author: 'K.A. - Zurigo',
          },
          {
            text: 'Grazie al lavoro con Danilo, ho già potuto affrontare e guarire diversi problemi, sia fisici che psicologici. Con poche domande mirate, ha colto in modo completo le questioni da affrontare e ha sciolto i blocchi in pochissimo tempo. Per me, il lavoro energetico richiede sempre un alto livello di fiducia. Con Danilo questa fiducia c’è stata al 100% dal primo momento. Ciò che mi ha stupito e convinto di più è che non dovevo fare nulla. Il desiderio e la volontà interiore di cambiare in meglio qualcosa nella mia vita è stato sufficiente per sentire immediatamente e chiaramente i risultati positivi. Non si beve acqua, non si ingoiano pillole e nemmeno si fanno esercizi ginnici per settimane e mesi. Caro Danilo, grazie mille per il tuo lavoro, che è un grande arricchimento per il mio benessere e che apprezzo molto. Così come ti stimo incredibilmente come persona.',
            author: 'K.G. - Lucerna',
          },
        ],
        column2: [
          {
            text: 'Ho conosciuto Danilo come un uomo aperto, empatico e benevolo. Il fatto che le persone siano felici, sane e soddisfatte gli sta a cuore. Mi ha sostenuto perché continuo a lottare con le vertigini, associate a vari problemi di salute. Nel mio caso, la medicina convenzionale ha dei limiti che Danilo può superare in un altro modo. Durante il suo lavoro è molto presente per me, in senso positivo. Non tutti i disturbi sono scomparsi (ancora). Tuttavia, la sensazione di affrontarli è un’altra, che è fondamentale. Il miglioramento è molto evidente. Ho recuperato molta qualità di vita e sono grato di aver incontrato quest’uomo.',
            author: 'P.B. - Svizzera',
          },
          {
            text: 'Ho conosciuto e apprezzato Danilo come una persona incredibilmente empatica e aperta. Con la sua grande conoscenza, mi ha aiutato nel mio processo per rilasciare i miei blocchi e dissolvere i miei vecchi schemi. Danilo, ti sono incredibilmente grato per il tuo sostegno e la tua natura calorosa.',
            author: 'A.H. - Svizzera',
          },
          {
            text: 'È passato del tempo e mi sono accorta di ulteriori miglioramenti. Il ginocchio che era sempre gonfio e dolorante non lo è più, ho cambiato postura e modo di camminare. Cammino molto di più rispetto a prima. Il colesterolo si è abbassato e ho voglia di cibo più sano. Ho i nervi più saldi, sotto stress resto lucida e non mi agito più emotivamente. Ho più gioia in generale rispetto alla vita, passo più tempo all’aperto. Ho smesso di bere alcol e sto meglio, sono più vigile, dormo meglio e al risveglio mi sento rigenerata.',
            author: 'M.B. - Svizzera',
          },
          {
            text: `Carissimo Danilo. In breve ti scrivo della mia esperienza..prima e dopo il lavoro esperienziale fatto due giorni fa’. Sono arrivata da te, sentendomi spossata..esausta..triste, con un sentimento di ansia..vuoto e angoscia, che mi trascinavo oramai da tempo..con l’energia al limite di riserva, che si potrebbe descrivere come un principio di depressione dell’anima. Interessante e’ stata la sensazione..di prenderne coscienza..di quanto la mente attraverso il corpo parla..percependo il collegamento..di questa connessione..e per vibrazione in disequilibrio..in prima istanza una sensazione strana di barcolaggio fisico generale..poi la chiarezza del tutto..immagini ed emozioni che appaiono..e riaffiorano..il peso che dalla somma del capo a poco a poco e’ sceso nel corpo..fino ai piedi e alle estremità delle mani..con l’istinto di scuoterli per liberare la stanchezza e la pesantezza captata e percepita durante la seduta..una vera liberazione di cose accumulate inutilmente prima!!! Lo scambio di battute erano un misto di pianto iniziale nel sentire il peso del vecchio..trasformandosi in risata..sempre piu’ consapevole e liberatoria. Mi sento ora liberata..definitivamente da quel passato che mi ero incollata per troppo tempo. Mi sento ora nuova e piu’ leggera..l’energia ha preso una nuova forma, pronta a crescere ed espandersi alle infinite possibilita’. Grazie di Cuore il Reset e’ stato compiuto. Ora posso Ricominciare..e Sorrido con Gioia!!! Infinitamente Grata per il tuo lavoro..anche se per me rimane sempre il mistero sul come e cosa..hai fatto per attuare questa trasformazione!!!`,
            author: 'N.W. - Svizzera',
          },
        ],
        column3: [
          {
            text: 'Non riesco quasi a trovare le parole per esprimere il piccolo miracolo che Danilo ha fatto su di me. Ha usato diverse tecniche energetiche, dalla sua già prodigiosa borsa degli attrezzi per lavorare a livello non materiale, che sono state sentite quasi istantaneamente a livello materiale. Sono venuta da Danilo con diversi problemi e dopo alcune brevi sessioni, ho sentito i miei problemi dipanarsi intorno a me e scivolare via. La mia salute è migliorata, il mio peso sta scendendo e le difficoltà che sto affrontando nella mia vita non sembrano più essere le grandi sfide che erano. In qualche modo, le porte si stanno aprendo davanti a me e sto facendo profondi cambiamenti di vita in modo fluido e facile. Il lavoro è sottile ma sorprendentemente efficace. Sono profondamente grata.',
            author: 'C.C. - Italia',
          },
          {
            text: 'Volevo farti sapere che il mal di schiena è passato. Piuttosto miracoloso, poiché mi stavo preparando a settimane di cure per la guarigione.',
            author: 'E.T. - Regno Unito',
          },
        ],
      },
      contact: {
        title: 'Contatti',
        text: 'Potete contattarmi senza impegno in modo da ricevere maggiori informazioni.',
      },
    },
    de: {
      hero: {
        main: 'Mit einem multidimensionalen Ansatz, sprachlichen und unterschwelligen Techniken begleite ich dich zu deinen gewünschten Zielen, die sich aus der Tiefe heraus entwickeln, an der Oberfläche realisiert werden und sich in neuen gefühlten Seins-Zuständen manifestieren.',
        secondary:
          'Respekt vor der Person, die Qualität des Zuhörens und die Fähigkeit, richtig zu intervenieren, zeichnen die Effizienz und Qualität meiner Arbeit aus.',
      },
      reviews: {
        column1: [
          {
            text: 'Die Zusammenarbeit mit Danilo ist humorvoll, klar und präzise. Er hat mir klare und direkte Fragen gestellt, um fokussiert an den Themen zu arbeiten, die ich verändern möchte. Innerhalb weniger Sitzungen durfte ich vertiefte Veränderungen auf körperlicher und seelischer Ebene erleben. Herzlichen Dank für deine Arbeit. Schön, dass es dich gibt.',
            author: 'S.B. - Switzerland',
          },
          {
            text: 'Danilo, wir kennen uns und sind ein Leben lang befreundet. Ich bin fasziniert von dem was du machst und bewundere deinen Weg. Ich möchte dir ein großes Dankeschön sagen für deine Arbeit an meinem Unterbewusstsein. Meine schlechten Gewohnheiten was den Konsum von Süssigkeiten betrifft, haben sich positiv verändert. Ich fühle mich frei, voller Energie und ich habe viel mehr Selbstkontrolle. Ich entscheide nun ganz bewusst, wann und wie viel. Mach diese Arbeit weiter für alle, die sich hilfesuchend an dich wenden.',
            author: 'B.B. - Switzerland',
          },
          {
            text: 'Danilo hat mir mit überzeugenden Übungen geholfen Muster in meiner Verhaltensweise zu erkennen und zu lösen. Ein wichtiger Schritt für die eigene Standortbestimmung! Herzlichen Dank für Deine wunderbare Arbeit.',
            author: 'K.A. - Zurich',
          },
          {
            text: 'Dank der Arbeit mit Danilo konnte ich schon verschiedene Themen, körperlicher wie auch seelischer Natur, angehen und heilen. Mit ein paar gezielten Fragen hat er die anzugehenden Themen umfänglich erfasst und Blockaden in kürzester Zeit aufgelöst. Energiearbeit setzt für mich immer eine grosse Vertrauenswürdigkeit voraus. Bei Danilo war dieses Vertrauen ab dem ersten Moment zu 100% da. Was mich besonders erstaunt und überzeugt hat; es brauchte kein Zutun von meiner Seite. Der Wunsch und die innere Bereitschaft, etwas im eigenen Leben zum Besseren zu verändern, reichte völlig aus um positive Resultate sofort und klar zu spüren. Keine Wässerchen trinken, keine Pillen schlucken und auch keine Turnübungen über Wochen und Monate. Lieber Danilo, Herzensdank für deine Arbeit, die eine grosse Bereicherung für mein Wohlbefinden ist und welche ich sehr schätze. Genauso wie ich dich als Mensch unglaublich schätze.',
            author: 'K.G. - Lucerne',
          },
        ],
        column2: [
          {
            text: 'Ich habe Danilo als offenen, empathischen und wohlwollenden Mann kennenlernen dürfen. Dass Menschen glücklich, gesund und zufrieden sind, ist ihm eine Herzensangelegenheit. Er unterstützt mich, da ich immer wieder mit Schwindel, verbunden mit verschiedenen gesundheitlichen Problemen, zu kämpfen habe. Die Schulmedizin stösst in meinem Fall an Grenzen, die Danilo auf einem anderen Weg überwinden kann. Während seiner Arbeit ist er mir sehr präsent, im positiven Sinn. Es sind (noch) nicht alle Beschwerden verschwunden. Das Gefühl, mit ihnen umzugehen ist jedoch ein ganz anderes, was entscheidend ist. Die Verbesserung ist stark spürbar. Ich erhielt viel Lebensqualität zurück und bin dankbar, diesem Mann begegnet zu sein.',
            author: 'P.B. - Switzerland',
          },
          {
            text: 'Ich habe Danilo als unglaublich emphatische, aufgestellte Person kennen und schätzen gelernt. Er hat mir bei meinem Eigenprozess mit seinem grossartigen Wissen geholfen, meine Blockaden zu lösen und meine alten Muster aufzulösen. Danilo ich bin dir unglaublich dankbar für deine Unterstützung, den Support und deine herzliche Art.',
            author: 'A.H. - Switzerland',
          },
          {
            text: 'Es ist einige Zeit vergangen, und ich habe weitere Verbesserungen festgestellt. Das Knie, das immer geschwollen und wund war, ist es nicht mehr, ich änderte meine Haltung und die Art, wie ich gehe. Ich laufe viel mehr als früher. Mein Cholesterinspiegel ist gesunken, und ich esse gesündere Lebensmittel. Ich habe festere Nerven, unter Stress bleibe ich bei klarem Verstand und werde nicht mehr emotional erregt. Ich habe im Allgemeinen mehr Freude am das Leben, ich verbringe mehr Zeit im Freien. Ich habe mit dem Alkoholkonsum aufgehört und fühle mich besser, ich bin wacher, schlafe besser und wenn ich aufwache, fühle ich mich regeneriert.',
            author: 'M.B. - Switzerland',
          },
          {
            text: `Lieber Danilo. Kurz gesagt, ich schreibe dir über meine Erfahrungen … vor und nach der Erfahrungsarbeit, die ich vor zwei Tagen gemacht habe. Ich bin zu Ihnen gekommen, weil ich mich erschöpft und traurig fühlte, mit einem Gefühl der Angst, der Leere und der Beklemmung, das ich seit langem mit mir herumschleppe, mit meiner Energie an der Grenze ihrer Reserven, was man als ein Prinzip der seelischen Depression beschreiben könnte. Interessant war das Gefühl, sich bewusst zu werden, wie sehr der Geist durch den Körper spricht, die Verbindung wahrzunehmen…diese Verbindung. Zuerst ein seltsames Gefühl des allgemeinen körperlichen Schwankens… dann die Klarheit von allem… Bilder und Emotionen, die auftauchen… und wieder hochkommen… das Gewicht, das von der Kopfspitze allmählich in den Körper hinabsteigt… bis hinunter zu den Füßen und den Extremitäten der Hände… mit dem Instinkt, sie zu schütteln, um die Müdigkeit und die Schwere zu befreien, die während der Sitzung aufgenommen und wahrgenommen wurden… eine echte Befreiung von Dingen, die sich zuvor nutzlos angesammelt hatten!!. Der Austausch war eine Mischung aus anfänglichen Tränen, als ich das Gewicht des alten spürte … und verwandelte sich in Lachen … das immer bewusster und befreiender wurde. Ich fühle mich jetzt befreit… definitiv von der Vergangenheit, die ich zu lange an mir kleben hatte. Ich fühle mich jetzt neu und leichter … die Energie hat eine neue Form angenommen, bereit zu wachsen und sich auf unendliche Möglichkeiten auszudehnen. Ich danke dir von ganzem Herzen, der Reset ist vollbracht. Jetzt kann ich wieder anfangen…und ich lächle vor Freude!!! Unendlich dankbar für deine Arbeit…auch wenn es für mich immer ein Rätsel bleibt, wie und was…du getan hast, um diese Transformation zu bewirken!!!`,
            author: 'N.W. - Switzerland',
          },
        ],
        column3: [
          {
            text: 'Ich kann kaum die Worte finden, um das kleine Wunder auszudrücken, dass Danilo an mir gearbeitet hat. Er benutzte verschiedene energetische Techniken, von seiner bereits erstaunlichen Werkzeugtasche für die Arbeit auf immaterieller Ebene, die fast sofort auf materieller Ebene zu spüren waren. Ich kam mit mehreren Problemen zu Danilo und nach ein paar kurzen Sitzungen, fühlte ich wie sich meine Probleme auflösten um mich herum wegrutschten. Meine Gesundheit verbesserte sich, mein Gewicht purzelte. Mein Leben würde leichter und unüberwindbare Herausforderungen wurden überwindbar. Es fliesst in meinen Leben wieder auf Positive weise. Seine Arbeit ist subtil, aber erstaunlich effektiv. Ich bin zutiefst dankbar.',
            author: 'C.C. - Italy',
          },

          {
            text: 'Ich wollte Ihnen mitteilen, dass die Rückenschmerzen nun verschwunden sind. Ziemlich wundersam, da ich mich auf eine wochenlange Heilung vorbereitet habe!',
            author: 'E.T. - UK',
          },
        ],
      },
      contact: {
        title: 'Kontakte',
        text: 'Sie können mich unverbindlich kontaktieren, um weitere Informationen zu erhalten.',
      },
    },
    en: {
      hero: {
        title: 'Contacts',
        main: `Using a multidimensional approach, linguistic and subliminal techniques, I accompany you towards your desired goals, which develop from the depths, are realised on the surface and manifest themselves in new perceived states of being.`,
        secondary: `Respect for the person, the quality of listening and the ability to intervene in a correct manner, distinguish the efficiency and quality of the work I do.`,
      },
      reviews: {
        column1: [
          {
            text: "The collaboration with Danilo is Humorous, clear and precise. He asked me clear and direct questions, to focus the work on the topics, that I want to change. Within a few sessions I was able to experience profound changes on the physical and emotional level. Thank you very much for your work. It's nice that you exist.",
            author: 'S.B. - Switzerland',
          },
          {
            text: "Danilo, we know each other and have been friends for a lifetime, I admire and fascinate your path, I just want to say a big THANK YOU for what you have done to my subconscious. My habits with sweets have changed, I feel free, more energetic and in control of myself, it is me who decide when and how much. I still don't know how you did it, but I don't care. Keep helping those who ask you.",
            author: 'B.B. - Switzerland',
          },
          {
            text: 'Danilo has helped me with convincing exercises to recognise and solve patterns in my behaviour. An important step for determining my own position! Many thanks for your wonderful work.',
            author: 'K.A. - Zurich',
          },
          {
            text: "Thanks to the work with Danilo, I have already been able to address and heal various issues, both physical and psychological. With a few targeted questions, he has comprehensively grasped the issues to be addressed and dissolved blockages in a very short time. For me, energy work always requires a high level of trust. With Danilo this trust was 100% there from the first moment. What amazed and convinced me the most was that I didn't have to do anything. The desire and the inner willingness to change something in my own life for the better was enough to feel positive results immediately and clearly. No need to drink water, swallow pills or do gymnastic exercises for weeks and months. Dear Danilo, thank you very much for your work, which is a great enrichment for my well-being and which I appreciate very much. Just as I value you incredibly as a person.",
            author: 'K.G. - Lucerne',
          },
        ],
        column2: [
          {
            text: 'I got to know Danilo as an open, empathetic and benevolent man. The fact that people are happy, healthy and satisfied is a matter close to his heart. He supports me because I keep struggling with dizziness, associated with various health problems. In my case, conventional medicine has limits that Danilo can overcome in another way. During his work he is very present to me, in a positive sense. Not all complaints have disappeared (yet). However, the feeling of dealing with them is quite another, which is crucial. The improvement is strongly noticeable. I got a lot of quality of life back and am grateful to have met this man.',
            author: 'P.B. - Switzerland',
          },
          {
            text: 'I have come to know and appreciate Danilo as an incredibly emphatic, upbeat person. He has helped me in my self-process with his great knowledge to release my blockages and dissolve my old patterns. Danilo I am incredibly grateful for your support and your warm nature.',
            author: 'A.H. - Switzerland',
          },
          {
            text: 'Some time has passed and I have noticed further improvements. The knee that was always swollen and sore is no so, I changed my posture and the way I walk. I walk much more than before. My cholesterol has dropped and I eat healthier food. I have firmer nerves, under stress I remain lucid and I no longer get emotionally agitated. I have more joy in general in life, I spend more time outdoors. I have stopped drinking alcohol and I feel better, I am more alert, I sleep better and when I wake up I feel regenerated.',
            author: 'M.B. - Switzerland',
          },
          {
            text: 'Dear Danilo. In short, I am writing to you about my experience…before and after the experiential work done two days ago. I came to you feeling exhausted, sad, with a feeling of anxiety, emptiness and anguish that I had been dragging around for a long time, with my energy at the limit of its reserve, which could be described as a principle of depression of the soul. Interesting was the sensation of becoming aware of it, of how much the mind speaks through the body, perceiving the connection..of this connection. In the first instance a strange sensation of general physical staggering…then the clarity of it all…images and emotions appearing…and resurfacing…the weight that from the sum of the head gradually descended in the body…down to the feet and the extremities of the hands…with the instinct to shake them to release the tiredness and heaviness picked up and perceived during the session…a real liberation of things accumulated unnecessarily before!!! The exchange of jokes were a mixture of initial tears in feeling the weight of the old … turning into laughter … more and more ‘conscious and liberating. I now feel liberated..definitively from that past that I had stuck to myself for too long. I now feel new and lighter..the energy has taken a new form, ready to grow and expand to infinite possibilities. Thank you from the Heart the Reset has been accomplished. Now I can start again..and I smile with joy!!! Infinitely Grateful for your work…even if for me it always remains a mystery as to how and what…you did to bring about this transformation!!!',
            author: 'N.W. - Switzerland',
          },
        ],
        column3: [
          {
            text: 'I can hardly find the words to express the small miracle that Danilo worked on me. He was using different energetic techniques, from his already prodigious tool bag for working on a non-material level, which were felt almost instantly on a material level. I came to Danilo with several problems and after a few short sessions, I felt my troubles unraveling around me and slipping away. My health improved, my weight is dropping off and the difficulties I am dealing with in my life no longer seem to be the great challenges that they were. Somehow, doors are opening in front of me and I am making deep life changes in a fluid easy manner. The work is subtle but astonishingly effective. I am deeply grateful',
            author: 'C.C. - Italy',
          },

          {
            text: 'I wanted to let you know that the back pain has gone now. Pretty miraculous as I was preparing for weeks of healing!',
            author: 'E.T. - UK',
          },
        ],
      },
      contact: {
        text: 'You can contact me without obligation for more information.',
      },
    },
  },
};
